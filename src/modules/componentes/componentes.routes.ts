...
import {
  ...
  getComponentesByAnalisisSchema,
} from './componentes.schema';

const router = Router();

router.get('/', componentesController.getAll.bind(componentesController));
router.get('/analisis/:analisisId', validate(getComponentesByAnalisisSchema), componentesController.getByAnalisisId.bind(componentesController));
